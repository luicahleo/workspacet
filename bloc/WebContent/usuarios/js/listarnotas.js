...
function mostrarDetalle(elemento, objetoDetalle, id) {
	if (objetoDetalle.error != null && objetoDetalle.error !="") {
		...
		elemento.innerHTML=
			"<p>Categoria: "+objetoDetalle.categoria+"</p>"+
			"<div style=\"background-color:"+objetoDetalle.color+";\">"+
			"<p class='textonota'>"+objetoDetalle.nota+"</p>"+
			"<p><img src='"+objetoDetalle.imagen+"' alt='Sin imagen' /></p>"+
			"</div>"+
			"<p><button class='boton' onclick='editarNota(event, " + id +");'>Editar</button>"+
			"<button class='boton' onclick='borrar(event, " + id +");'>Borrar</button></p>";
